...
const PRESETS = [
  ...
  { label: "240p (426x240)", w: 426, h: 240 },
];

export default function ResizePanel({ video }: ResizePanelProps) {
  ...
  const handleResize = useCallback(async () => {
    ...
    await ffmpeg.run(async (instance) => {
      ...
      const scale = maintainAspect ? `scale=${width}:${height}:force_original_aspect_ratio=decrease,pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2` : `scale=${width}:${height}`;
  ...
  }, [video.data, video.name, width, height, maintainAspect]);
